"""
This example demonstrates how to set up and use an in-memory FlashMCP proxy.

It illustrates the pattern:
1. Create an original FlashMCP server with some tools.
2. Create a Client that connects to this original server (in-memory).
3. Create a proxy FlashMCP server using FlashMCP.from_client(), passing it the client from step 2.
...
async def main():
    print("--- In-Memory FlashMCP Proxy Example ---")
    ...
    original_server = FlashMCP("OriginalEchoServer")
    ...
    print(
        f"          (Using FlashMCP.from_client, passing it the client from Step 2 that targets '{original_server.name}')"
    )
    proxy_server = FlashMCP.from_client(client_to_original, name="InMemoryProxy")
    print(
