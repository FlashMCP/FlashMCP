...
2. Create a proxy FlashMCP server using ``FlashMCP.as_proxy(original_server)``.
...
async def main():
    ...
    print(
        f"          (Using FlashMCP.as_proxy to wrap '{original_server.name}' directly)"
    )
    proxy_server = FlashMCP.as_proxy(original_server, name="InMemoryProxy")
    print(
