...
import FlashMCP.cli.run
from FlashMCP.cli import cli
...
def mock_console():
    ...
    with patch("FlashMCP.cli.cli.console") as mock_console:
        yield mock_console
...
def mock_logger():
    ...
    with patch("FlashMCP.cli.cli.logger") as mock_logger:
        yield mock_logger
...
class TestHelperFunctions:
    def test_parse_file_path_simple(self):
        ...
        with (
            ...
            path, obj = FlashMCP.cli.run.parse_file_path("file.py")
    ...
    def test_parse_file_path_with_object(self):
        ...
        with (
            ...
            path, obj = FlashMCP.cli.run.parse_file_path("file.py:server")
            assert path == Path("file.py")
            assert obj == "server"
    ...
    def test_parse_file_path_windows(self):
        ...
        with (
            ...
            path, obj = FlashMCP.cli.run.parse_file_path("C:/path/file.py:server")
    ...
    def test_parse_file_path_not_file(self, mock_exit):
        ...
        with (
            ...
            patch("pathlib.Path.expanduser") as mock_expanduser,
            patch("pathlib.Path.resolve") as mock_resolve,
            patch("FlashMCP.cli.run.logger") as mock_logger,
        ):
            mock_exists.return_value = True
            ...
            FlashMCP.cli.run.parse_file_path("directory")
...
class TestRunCommand:
    ...
    def test_run_command_success(self, temp_python_file):
        ...
        with (
            patch("FlashMCP.cli.run.parse_file_path") as mock_parse,
            patch("FlashMCP.cli.run.import_server") as mock_import,
            patch("FlashMCP.cli.run.logger") as mock_logger,
        ):
