...
from FlashMCP.exceptions import ClientError, NotFoundError, ToolError
...
class TestCreateServer:
    async def test_create_server(self):
        mcp = FlashMCP(instructions="Server instructions")
        assert mcp.name == "FlashMCP"
        assert mcp.instructions == "Server instructions"

    async def test_non_ascii_description(self):
        """Test that FlashMCP handles non-ASCII characters in descriptions correctly"""
        mcp = FlashMCP()
...
class TestServerTools:
    async def test_add_tool_exists(self, tool_server: FlashMCP):
        assert "add" in [t.name for t in await tool_server._mcp_list_tools()]

    async def test_list_tools(self, tool_server: FlashMCP):
        assert len(await tool_server._mcp_list_tools()) == 6

    async def test_call_tool(self, tool_server: FlashMCP):
        result = await tool_server._mcp_call_tool("add", {"x": 1, "y": 2})
        assert isinstance(result[0], TextContent)
        assert result[0].text == "3"

    async def test_call_tool_as_client(self, tool_server: FlashMCP):
        async with Client(tool_server) as client:
            result = await client.call_tool("add", {"x": 1, "y": 2})
            assert isinstance(result[0], TextContent)
            assert result[0].text == "3"

    async def test_call_tool_error(self, tool_server: FlashMCP):
        with pytest.raises(ToolError):
            await tool_server._mcp_call_tool("error_tool", {})

    async def test_call_tool_error_as_client(self, tool_server: FlashMCP):
        async with Client(tool_server) as client:
            with pytest.raises(Exception):
                await client.call_tool("error_tool", {})

    async def test_call_tool_error_as_client_raw(self, tool_server: FlashMCP):
        async with Client(tool_server) as client:
    ...
    async def test_tool_returns_list(self, tool_server: FlashMCP):
        result = await tool_server._mcp_call_tool("list_tool", {})
        assert isinstance(result[0], TextContent)
        assert result[0].text == '["x", 2]'

    async def test_tool_image_helper(self, tool_server: FlashMCP, tmp_path: Path):
        # Create a test image
    ...
    async def test_tool_mixed_content(self, tool_server: FlashMCP):
    ...
    async def test_tool_mixed_list_with_image(
        self, tool_server: FlashMCP, tmp_path: Path
    ):
    ...
    async def test_resource_multiple_params(self):
        ...
        async with Client(mcp) as client:
            result = await client.read_resource(
                AnyUrl("resource://cursor/FlashMCP/data")
            )
            assert isinstance(result[0], TextResourceContents)
            assert result[0].text == "Data for cursor/FlashMCP"
...
class TestServerPrompts:
    """Test prompt functionality in FlashMCP server."""
