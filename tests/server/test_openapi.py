...
class TestTools:
    async def test_list_tools(self, FlashMCP_server: FlashMCPOpenAPI):
        """
        By default, tools exclude GET methods
        """
        tools = await FlashMCP_server._mcp_list_tools()
        assert len(tools) == 2
    ...
    async def test_call_create_user_tool(
        self, FlashMCP_server: FlashMCPOpenAPI, api_client
    ):
        """
        The tool created by the OpenAPI server should be the same as the original
        """
        tool_response = await FlashMCP_server.call_tool(
        ...
        user_response = await FlashMCP_server._mcp_read_resource(
...
class TestResources:
    async def test_list_resources(self, FlashMCP_server: FlashMCPOpenAPI):
        """
        By default, resources exclude GET methods without parameters
        """
        resources = await FlashMCP_server._mcp_list_resources()
        assert len(resources) == 1
        assert resources[0].uri == AnyUrl("resource://openapi/get_users_users_get")
        assert resources[0].name == "get_users_users_get"

    async def test_get_resource(
        self, FlashMCP_server: FlashMCPOpenAPI, api_client, users_db: dict[int, User]
    ):
        """
        ...
        resource_response = await FlashMCP_server._mcp_read_resource(
...
class TestResourceTemplates:
    async def test_list_resource_templates(self, FlashMCP_server: FlashMCPOpenAPI):
        """
        By default, resource templates exclude GET methods without parameters
        """
        resource_templates = await FlashMCP_server._mcp_list_resource_templates()
...
class TestPrompts:
    async def test_list_prompts(self, FlashMCP_server: FlashMCPOpenAPI):
        """
        By default, there are no prompts.
        """
        prompts = await FlashMCP_server._mcp_list_prompts()
...
class TestTagTransfer:
    ...
    async def test_tags_transferred_to_tools(self, FlashMCP_server: FlashMCPOpenAPI):
        ...
        tools = FlashMCP_server._tool_manager.list_tools()
    ...
    async def test_tags_transferred_to_resources(self, FlashMCP_server: FlashMCPOpenAPI):
        """Test that tags from OpenAPI routes are correctly transferred to Resources."""
        # Get internal resources directly
        resources = FlashMCP_server._resource_manager.list_resources()
    ...
    async def test_tags_transferred_to_resource_templates(
        self, FlashMCP_server: FlashMCPOpenAPI
    ):
        ...
        templates = FlashMCP_server._resource_manager.list_templates()
