...
async def test_mount_basic_functionality():
    ...
    main_app = FlashMCP("MainApp")
    sub_app = FlashMCP("SubApp")
...
async def test_mount_multiple_apps():
    ...
    main_app = FlashMCP("MainApp")
    weather_app = FlashMCP("WeatherApp")
    news_app = FlashMCP("NewsApp")
...
async def test_mount_combines_tools():
    ...
    # Create apps
    main_app = FlashMCP("MainApp")
    first_app = FlashMCP("FirstApp")
    second_app = FlashMCP("SecondApp")
...
async def test_mount_with_prompts():
    ...
    # Create apps
    main_app = FlashMCP("MainApp")
    assistant_app = FlashMCP("AssistantApp")
...
async def test_mount_multiple_prompts():
    ...
    # Create apps
    main_app = FlashMCP("MainApp")
    python_app = FlashMCP("PythonApp")
    sql_app = FlashMCP("SQLApp")
...
async def test_mount_lifespan():
    ...
    async def lifespan(app: FlashMCP):
    ...
    main_app = FlashMCP("MainApp", lifespan=lifespan)
    sub_app = FlashMCP("SubApp", lifespan=lifespan)
    sub_app_2 = FlashMCP("SubApp2", lifespan=lifespan)
...
async def test_first_level_mounting_with_custom_name():
    ...
    service_app = FlashMCP("ServiceApp")
    provider_app = FlashMCP("ProviderApp")
...
async def test_mount_with_proxy_tools():
    ...
    main_app.import_server("api", await FlashMCP.as_proxy(api_app))
