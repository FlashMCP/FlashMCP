...
from FlashMCP import Client, FlashMCP
from FlashMCP.server.openapi import OpenAPITool, RouteMap, RouteType
from FlashMCP.utilities.openapi import HTTPRoute, ParameterInfo
...
async def test_FlashMCP_from_openapi(array_path_spec, mock_client):
    """Test creating FlashMCP from OpenAPI spec with array path parameter."""
    # Create FlashMCP from the spec
    mcp = FlashMCP.from_openapi(array_path_spec, client=mock_client)
...
async def test_array_query_param_with_fastapi():
    """Test array query parameters using FastAPI and FlashMCP.from_fastapi integration."""
    ...
    # Create a FlashMCP server from the FastAPI app
    mcp = FlashMCP.from_fastapi(
        app,
        route_maps=[
