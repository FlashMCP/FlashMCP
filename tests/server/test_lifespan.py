"""Tests for lifespan functionality in both low-level and FlashMCP servers."""
...
from FlashMCP.utilities.tests import run_server_in_process
...
async def test_FlashMCP_server_lifespan():
    """Test that lifespan works in FlashMCP server."""

    @asynccontextmanager
    async def test_lifespan(server: FlashMCP) -> AsyncIterator[dict]:
        """Test lifespan context that tracks startup/shutdown."""
        context = {"started": False, "shutdown": False}
        try:
            context["started"] = True
    ...
    server = FlashMCP("test", lifespan=test_lifespan)
...
async def test_missing_lifespan_logs_informative_error(tmp_path: Path):
    ...
    assert (
        "FlashMCP's StreamableHTTPSessionManager task group was not initialized"
        in log_content
    )
    assert "lifespan=mcp_app.lifespan" in log_content
    assert "goFlashMCP.com/deployment/asgi" in log_content
