...
def FlashMCP_server():
    ...
    async def sample_with_system_prompt(message: str, context: Context) -> str:
        result = await context.sample("Hello, world!", system_prompt="You love FlashMCP")
        return cast(TextContent, result).text
...
async def test_simple_sampling(FlashMCP_server: FlashMCP):
    def sampling_handler(
        messages: list[SamplingMessage], params: SamplingParams, ctx: RequestContext
    ) -> str:
        return "This is the sample message!"

    async with Client(FlashMCP_server, sampling_handler=sampling_handler) as client:
...
async def test_sampling_with_system_prompt(FlashMCP_server: FlashMCP):
    def sampling_handler(
        messages: list[SamplingMessage], params: SamplingParams, ctx: RequestContext
    ) -> str:
    ...
    async with Client(FlashMCP_server, sampling_handler=sampling_handler) as client:
        ...
        reply = cast(TextContent, result.content[0])
        assert reply.text == "You love FlashMCP"


async def test_sampling_with_messages(FlashMCP_server: FlashMCP):
    def sampling_handler(
        messages: list[SamplingMessage], params: SamplingParams, ctx: RequestContext
    ) -> str:
