...
def mounted_resources_server():
    """Fixture that creates a FlashMCP server with mounted resources."""
    # Create the main server
    main_server = FlashMCP("MainServer")

    # Create sub-app with its own resources
    sub_app = FlashMCP("SubAppServer")
...
async def test_mounted_resources(mounted_resources_server):
    ...
    client = Client(transport=FlashMCPTransport(mounted_resources_server))
