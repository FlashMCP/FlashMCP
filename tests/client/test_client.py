...
async def test_list_tools_mcp(FlashMCP_server):
...
async def test_call_tool_mcp(FlashMCP_server):
...
async def test_list_resources_mcp(FlashMCP_server):
...
async def test_list_prompts_mcp(FlashMCP_server):
...
async def test_read_resource_mcp(FlashMCP_server):
...
async def test_initialize_result_connected(FlashMCP_server):
...
async def test_initialize_result_disconnected(FlashMCP_server):
...
async def test_list_resource_templates_mcp(FlashMCP_server):
...
class TestTimeout:
    async def test_timeout(self, FlashMCP_server: FlashMCP):
        async with Client(
            transport=FlashMCPTransport(FlashMCP_server), timeout=0.05
        ) as client:
            with pytest.raises(
                McpError,
    ...
    async def test_timeout_tool_call(self, FlashMCP_server: FlashMCP):
        async with Client(transport=FlashMCPTransport(FlashMCP_server)) as client:
    ...
    async def test_timeout_tool_call_overrides_client_timeout(
        ...
        async with Client(
            transport=FlashMCPTransport(FlashMCP_server),
            timeout=2,
        ) as client:
            with pytest.raises(McpError):
                await client.call_tool("sleep", {"seconds": 0.1}, timeout=0.01)
