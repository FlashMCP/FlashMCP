...
async def test_progress_handler(FlashMCP_server: FlashMCP):
    async with Client(FlashMCP_server, progress_handler=progress_handler) as client:
        await client.call_tool("progress_tool", {})

    assert PROGRESS_MESSAGES == EXPECTED_PROGRESS_MESSAGES


async def test_progress_handler_can_be_supplied_on_tool_call(FlashMCP_server: FlashMCP):
...
async def test_progress_handler_supplied_on_tool_call_overrides_default(
    FlashMCP_server: FlashMCP,
):
    async def bad_progress_handler(*args, **kwargs):
        1 / 0

    async with Client(FlashMCP_server, progress_handler=bad_progress_handler) as client:
        await client.call_tool("progress_tool", {}, progress_handler=progress_handler)
