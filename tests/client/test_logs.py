...
class TestClientLogs:
    async def test_log(self, FlashMCP_server: FlashMCP):
        log_handler = LogHandler()
        async with Client(FlashMCP_server, log_handler=log_handler.handle_log) as client:
    ...
    async def test_echo_log(self, FlashMCP_server: FlashMCP):
        log_handler = LogHandler()
        async with Client(FlashMCP_server, log_handler=log_handler.handle_log) as client:
