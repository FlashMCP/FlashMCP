...
class TestClientRoots:
    ...
    async def test_invalid_roots(self, FlashMCP_server: FlashMCP, roots: list[str]):
        """
        Roots must be URIs
        """
        with pytest.raises(ValueError, match="Input should be a valid URL"):
            async with Client(FlashMCP_server, roots=roots):
                pass

    @pytest.mark.parametrize("roots", [["https://x.com"]])
    async def test_invalid_urls(self, FlashMCP_server: FlashMCP, roots: list[str]):
        """
    ...
    async def test_valid_roots(self, FlashMCP_server: FlashMCP, roots: list[str]):
        async with Client(FlashMCP_server, roots=roots) as client:
