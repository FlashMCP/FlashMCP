...
The [Model Context Protocol (MCP)](https://modelcontextprotocol.io) is a new, standardized way to provide context and tools to your LLMs, and FlashMCP makes building MCP servers and clients simple and intuitive. Create tools, expose resources, define prompts, and connect components with clean, Pythonic code.
...
# Specify dependencies needed when deployed via `FlashMCP install`
...
Decorate synchronous or asynchronous functions with `@mcp.tool()`. FlashMCP automatically generates the necessary MCP schema based on type hints and docstrings. Pydantic models can be used for complex inputs.
...
Gain access to MCP server capabilities *within* your tool or resource functions by adding a parameter type-hinted with `FlashMCP.Context`.

```python
from FlashMCP import Context, FlashMCP

mcp = FlashMCP("Context Demo")
...
Easily handle image input and output using the `FlashMCP.Image` helper class.
...
```
FlashMCP handles the conversion to/from the base64-encoded format required by the MCP protocol.


### MCP Clients

The `Client` class lets you interact with any MCP server (not just FlashMCP ones) from Python code:
...
You can connect to servers using any supported transport protocol (Stdio, SSE, FlashMCP, etc.). If you don't specify a transport, the `Client` class automatically attempts to detect an appropriate one from your connection string or server object.
...
These methods correspond directly to MCP protocol operations, making it easy to interact with any MCP-compatible server (not just FlashMCP ones).

#### Transport Options

FlashMCP supports various transport protocols for connecting to MCP servers:
...
# Connect directly to a FlashMCP server object in the same process
from your_app import mcp_server
async with Client(FlashMCPTransport(mcp_server)) as client:
    # Use client here...
```
...
- `FlashMCPTransport`: Connect directly to a FlashMCP server object
- `WSTransport`: Connect via WebSockets

In addition, if you pass a connection string or `FlashMCP` server object to the `Client` constructor, it will try to automatically detect the appropriate transport.
...
from FlashMCP import Client, Context, FlashMCP
from FlashMCP.client.sampling import RequestContext, SamplingMessage, SamplingParams

# -- SERVER SIDE --
# Create a server that requests LLM completions from the client

mcp = FlashMCP("Sampling Example")
...
FlashMCP exposes the MCP roots functionality, allowing clients to specify which file system roots they can access. This creates a secure boundary for tools that need to work with files. Note that the server must account for client roots explicitly.

```python
from FlashMCP import Client, RootsList
...
Building on the core concepts, FlashMCP v2 introduces powerful features for more complex scenarios:


### Proxy Servers

Create a FlashMCP server that acts as an intermediary, proxying requests to another MCP endpoint (which could be a server or another client connection).
...
proxy = FlashMCP.from_client(proxy_client, name="Stdio-to-SSE Proxy")

if __name__ == "__main__":
    proxy.run(transport='sse')
```

`FlashMCP.from_client` is a class method that connects to the target, discovers its capabilities, and dynamically builds the proxy server instance.



### Composing MCP Servers

Structure larger MCP applications by creating modular FlashMCP servers and "mounting" them onto a parent server. This automatically handles prefixing for tool names and resource URIs, preventing conflicts.

```python
from FlashMCP import FlashMCP

# --- Weather MCP ---
weather_mcp = FlashMCP("Weather Service")
...
# --- News MCP ---
news_mcp = FlashMCP("News Service")
...
Leverage your existing web APIs by automatically generating FlashMCP servers from them.
...
mcp_server = FlashMCP.from_fastapi(fastapi_app)
...
mcp_server = FlashMCP.from_openapi(openapi_spec, client=http_client)
...
doesn't specify the format in any way. FlashMCP will forward the server's `stderr`
...
Use `FlashMCP dev` for an interactive testing environment with the MCP Inspector.

```bash
FlashMCP dev your_server_file.py
# With temporary dependencies
FlashMCP dev your_server_file.py --with pandas --with numpy
# With local package in editable mode
FlashMCP dev your_server_file.py --with-editable .
```

### Claude Desktop Integration (For Regular Use)

Use `FlashMCP install` to set up your server for persistent use within the Claude Desktop app. It handles creating an isolated environment using `uv`.

```bash
FlashMCP install your_server_file.py
# With a custom name in Claude
FlashMCP install your_server_file.py --name "My Analysis Tool"
# With extra packages and environment variables
FlashMCP install server.py --with requests -v API_KEY=123 -f .env
```
...
```python
if __name__ == "__main__":
    mcp.run() # Assuming 'mcp' is your FlashMCP instance
```
Run with:
```bash
...
If your `FlashMCP` instance is not named `mcp`, `server`, or `app`, specify it using `file:object` syntax for the `dev` and `install` commands:

```bash
FlashMCP dev my_module.py:my_mcp_instance
FlashMCP install api.py:api_app
```
...
*   `mount_example.py`: Mounting multiple FlashMCP servers.
...
1.  Clone: `git clone https://github.com/jlowin/FlashMCP.git && cd FlashMCP`
...
4.  Open a pull request against the `main` branch of `jlowin/FlashMCP`.
