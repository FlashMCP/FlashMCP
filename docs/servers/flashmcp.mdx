---
title: The FlashMCP Server
sidebarTitle: FlashMCP Server
description: Learn about the core FlashMCP server class and how to run it.
icon: server
---

import { VersionBadge } from "/snippets/version-badge.mdx"

The central piece of a FlashMCP application is the `FlashMCP` server class. This class acts as the main container for your application's tools, resources, and prompts, and manages communication with MCP clients.
...
mcp_with_instructions = FlashMCP(
    name="HelpfulAssistant",
    instructions="This server provides data analysis tools. Call get_average() to analyze numerical data."
)
```

The `FlashMCP` constructor accepts several arguments:

*   `name`: (Optional) A human-readable name for your server. Defaults to "FlashMCP".
...
FlashMCP servers expose several types of components to the client:
...
FlashMCP servers need a transport mechanism to communicate with clients. In the MCP protocol, servers typically run as separate processes that clients connect to.
...
While this pattern is technically optional when using FlashMCP's CLI, it's considered a best practice for maximum compatibility with all MCP clients.

### Transport Options

FlashMCP supports two transport mechanisms:
...
For long-running servers that serve multiple clients, FlashMCP supports SSE:
...
Transport parameters passed to `run()` or `run_sse_async()` override any settings defined when creating the FlashMCP instance. The most common parameters for SSE transport are:
...
Under the hood, FlashMCP's `run()` method accepts arbitrary keyword arguments (`**transport_kwargs`) that are passed to the transport-specific run methods:
...
The FlashMCP CLI provides a convenient way to run servers:

```bash
# Run a server (defaults to stdio transport)
FlashMCP run my_server.py:mcp

# Explicitly specify a transport
FlashMCP run my_server.py:mcp --transport sse

# Configure SSE transport with host and port
FlashMCP run my_server.py:mcp --transport sse --host 127.0.0.1 --port 8888

# With log level
FlashMCP run my_server.py:mcp --transport sse --log-level DEBUG
```

The CLI can dynamically find and run FlashMCP server objects in your files, but including the `if __name__ == "__main__":` block ensures compatibility with all clients.

## Composing Servers

<VersionBadge version="2.2.0" />

FlashMCP supports composing multiple servers together using `import_server` (static copy) and `mount` (live link). This allows you to organize large applications into modular components or reuse existing servers.
...
main = FlashMCP(name="Main")
sub = FlashMCP(name="Sub")
...
FlashMCP can act as a proxy for any MCP server (local or remote) using `FlashMCP.from_client`, letting you bridge transports or add a frontend to existing servers. For example, you can expose a remote SSE server locally via stdio, or vice versa.
...
proxy = FlashMCP.from_client(backend, name="ProxyServer")
# Now use the proxy like any FlashMCP server
```

## Server Configuration

Server behavior, like transport settings (host, port for SSE) and how duplicate components are handled, can be configured via `ServerSettings`. These settings can be passed during `FlashMCP` initialization, set via environment variables (prefixed with `FASTMCP_SERVER_`), or loaded from a `.env` file.
...
All of these can be configured directly as parameters when creating the `FlashMCP` instance.

### Custom Tool Serialization

<VersionBadge version="2.2.7" />

By default, FlashMCP serializes tool return values to JSON when they need to be converted to text. You can customize this behavior by providing a `tool_serializer` function when creating your server:
...
mcp = FlashMCP(name="MyServer", tool_serializer=yaml_serializer)
...
FlashMCP inherits support for OAuth 2.0 authentication from the MCP protocol, allowing servers to protect their tools and resources behind authentication.
