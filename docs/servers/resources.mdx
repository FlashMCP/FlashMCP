...
<Warning>
Please note: the Model Context Protocol URI standard follows RFC 6570, which does not include support for wildcard parameters. FlashMCP extends the template syntax to support wildcards (`{param*}`), and because template matching happens entirely in the FlashMCP server, it is not expected that these wildcards will cause compatibility issues with other MCP implementations. However, this can not be guaranteed.
</Warning>
