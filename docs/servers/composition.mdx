...
description: Combine multiple FlashMCP servers into a single, larger application using mounting and importing.
icon: puzzle-piece
---
...
-   **Teamwork**: Different teams can work on separate FlashMCP servers that are later combined.
...
FlashMCP supports [MCP proxying](/patterns/proxy), which allows you to mirror a local or remote server in a local FlashMCP instance. Proxies are fully compatible with both importing and mounting.
...
The `import_server()` method copies all components (tools, resources, templates, prompts) from one `FlashMCP` instance (the *subserver*) into another (the *main server*). A `prefix` is added to avoid naming conflicts.
...
# Define main server
main_mcp = FlashMCP(name="MainApp")
...
# Define subserver
dynamic_mcp = FlashMCP(name="DynamicService")
...
main_mcp = FlashMCP(name="MainAppLive")
...
FlashMCP automatically uses proxy mounting when the mounted server has a custom lifespan, but you can override this behavior with the `as_proxy` parameter.

#### Interaction with Proxy Servers

When using `FlashMCP.as_proxy()` to create a proxy server, mounting that server will always use proxy mounting:

```python
# Create a proxy for a remote server
remote_proxy = FlashMCP.as_proxy(Client("http://example.com/mcp"))
...
This is the default format since FlashMCP 2.4. This format is recommended because it avoids issues with URI protocol restrictions (like underscores not being allowed in protocol names).
...
This was the default format in FlashMCP before 2.4. While still supported, it's not recommended for new code as it can cause problems with prefix names that aren't valid in URI protocols.
...
```python
import FlashMCP
FlashMCP.settings.settings.resource_prefix_format = "protocol" 
```
...
server = FlashMCP("LegacyServer", resource_prefix_format="protocol")

# Create a server that uses new path format
server = FlashMCP("NewServer", resource_prefix_format="path")
```
