...
Tools are the core building blocks that allow your LLM to interact with external systems, execute code, and access data that isn't in its training data. In FlashMCP, tools are Python functions exposed to LLMs through the MCP protocol.

## What Are Tools?

Tools in FlashMCP transform regular Python functions into capabilities that LLMs can invoke during conversations. When an LLM decides to use a tool:

1.  It sends a request with parameters based on the tool's schema.
2.  FlashMCP validates these parameters against your function's signature.
...
mcp = FlashMCP(name="CalculatorServer")
...
When this tool is registered, FlashMCP automatically:
...
<Tip>
Functions with `*args` or `**kwargs` are not supported as tools. This restriction exists because FlashMCP needs to generate a complete parameter schema for the MCP protocol, which isn't possible with variable argument lists.
</Tip>
...
2. Enable FlashMCP to validate input data from clients
...
FlashMCP supports a wide range of type annotations, including all Pydantic types:
...
FlashMCP follows Python's standard function parameter conventions. Parameters without default values are required, while those with default values are optional.
...
While FlashMCP infers the name and description from your function, you can override these and add tags using arguments to the `@mcp.tool` decorator:
...
FlashMCP seamlessly supports both standard (`def`) and asynchronous (`async def`) functions as tools.
...
FlashMCP automatically converts the value returned by your function into the appropriate MCP content format for the client:
...
- **`FlashMCP.Image`**: A helper class for easily returning image data. Sent as `ImageContent`.
- **`None`**: Results in an empty response (no content is sent back to the client).

FlashMCP will attempt to serialize other types to a string if possible.

<Tip>
At this time, FlashMCP responds only to your tool's return *value*, not its return *annotation*.
</Tip>
...
def generate_image(width: int, height: int, color: str) -> Image:
    ...
    # Return using FlashMCP's Image helper
...
If your tool encounters an error, you can raise a standard Python exception (`ValueError`, `TypeError`, `FileNotFoundError`, custom exceptions, etc.) or a FlashMCP `ToolError`.
...
FlashMCP supports these standard annotations:
...
FlashMCP supports a wide variety of parameter types to give you flexibility when designing your tools.

FlashMCP generally supports all types that Pydantic supports as fields, including all Pydantic custom types. This means you can use any type that can be validated and parsed by Pydantic in your tool parameters.

FlashMCP supports **type coercion** when possible. This means that if a client sends data that doesn't match the expected type, FlashMCP will attempt to convert it to the appropriate type. For example, if a client sends a string for a parameter annotated as `int`, FlashMCP will attempt to convert it to an integer. If the conversion is not possible, FlashMCP will return a validation error.
...
These types provide clear expectations to the LLM about what values are acceptable and allow FlashMCP to validate inputs properly. Even if a client provides a string like "42", it will be coerced to an integer for parameters annotated as `int`.

### Date and Time Types

FlashMCP supports various date and time types from the `datetime` module:
...
FlashMCP supports all standard Python collection types:
...
- FlashMCP automatically coerces the string value into the appropriate Enum object
...
When you annotate a parameter as `bytes`, FlashMCP will:
- Convert raw strings directly to bytes
- Validate that the input can be properly represented as bytes

FlashMCP does not automatically decode base64-encoded strings for bytes parameters. If you need to accept base64-encoded data, you should handle the decoding manually as shown below.
...
When a client sends a string path, FlashMCP automatically converts it to a `Path` object.
...
When a client sends a string UUID (e.g., "123e4567-e89b-12d3-a456-426614174000"), FlashMCP automatically converts it to a `UUID` object.
...
FlashMCP supports robust parameter validation through Pydantic's `Field` class. This is especially useful to ensure that input values meet specific requirements beyond just their type.
...
When a client sends invalid data, FlashMCP will return a validation error explaining why the parameter failed validation.
...
You can control how the FlashMCP server behaves if you try to register multiple tools with the same name. This is configured using the `on_duplicate_tools` argument when creating the `FlashMCP` instance.
...
FlashMCP 1.0 and < 2.2.10 relied on a crutch that attempted to work around LLM limitations by automatically parsing stringified JSON in tool arguments (e.g., converting `"[1,2,3]"` to `[1,2,3]`). As of FlashMCP 2.2.10, this behavior is disabled by default because it circumvents type validation and can lead to unexpected type coercion issues (e.g. parsing "true" as a bool and attempting to call a tool that expected a string, which would fail type validation).
