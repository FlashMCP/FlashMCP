...
FlashMCP supports a wide variety of parameter types to give you flexibility when designing your tools. When clients send parameters, FlashMCP will attempt to coerce values into the appropriate type when possible (for example, parsing JSON strings into structured types).
...
For structured data collections, FlashMCP supports standard Python collection types:
...
Collection types can be nested and combined to represent complex data structures. If a client sends a JSON string like `"[1.5, 2.5, 3.5]"` for a `list[float]` parameter, FlashMCP will automatically parse and convert it.
