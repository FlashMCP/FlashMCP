...
When defining FlashMCP [tools](/servers/tools), your functions might need to interact with the underlying MCP session or access server capabilities. FlashMCP provides the `Context` object for this purpose.
...
To use the context object within your tool function, simply add a parameter to your function signature and type-hint it as `Context`. FlashMCP will automatically inject the context instance when your tool is called.
...
Read data from resources registered with your FlashMCP server. This allows tools to access files, configuration, or dynamically generated content.
