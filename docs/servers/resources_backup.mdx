...
async def get_user_profile(user_id: int) -> dict:
    ...
    # FlashMCP uses Pydantic to auto-convert the string URI part to int
    if user_id == 1:
...
3.  **Request & Matching:** A client requests a specific URI, e.g., `data://weather/london`. FlashMCP matches this to the `data://weather/{city}` template.
