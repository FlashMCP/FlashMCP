...
*   **Use Case:** Connecting to persistent MCP servers exposed over HTTP/S using FlashMCP's `mcp.run(transport="streamable-http")` mode.
...
*   **Use Case:** Connecting to MCP servers using Server-Sent Events, often using FlashMCP's `mcp.run(transport="sse")` mode.
...
*   **Use Case:** Running a Python-based MCP server script (like one using FlashMCP or the base `mcp` library) in a subprocess.
...
<Warning>
The server script (`my_mcp_server.py` in the example) *must* include logic to start the MCP server and listen on stdio, typically via `mcp.run()` or `FlashMCP.server.run()`. The `Client` only launches the script; it doesn't inject the server logic.
</Warning>
...
*   **Use Case:** Connecting directly to a `FlashMCP` server instance running in the *same Python process*.

This is extremely useful for:
*   **Testing:** Writing unit or integration tests for your FlashMCP server without needing subprocesses or network connections.
...
# Option B: Explicit
transport_explicit = FlashMCPTransport(mcp=server)
client_explicit = Client(transport_explicit)
...
*   **Local Development/Testing:** Use `PythonStdioTransport` (inferred from `.py` files) or `FlashMCPTransport` (for same-process testing).
