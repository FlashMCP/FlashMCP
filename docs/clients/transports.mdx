...
*   **Use Case:** Connecting to persistent MCP servers exposed over HTTP/S, often using FlashMCP's `mcp.run(transport="sse")` mode.
...
*   **Class:** `FlashMCP.client.transports.WSTransport`
...
```python
from FlashMCP import Client
from FlashMCP.client.transports import WSTransport
