...
description: Automatically create FlashMCP servers directly from FastAPI applications.
icon: square-bolt
---

If you build your APIs using the popular [FastAPI](https://fastapi.tiangolo.com/) framework, FlashMCP offers a seamless way to expose your FastAPI application as an MCP server. This leverages the OpenAPI integration internally but simplifies the setup significantly.

## The Goal: FastAPI App -> MCP Server

FastAPI automatically generates an OpenAPI specification for your application. FlashMCP uses this built-in capability to create an MCP server that mirrors your API routes.
...
Use the `FlashMCP.from_fastapi()` class method. You only need your FastAPI `app` instance.

```python
import asyncio
from fastapi import FastAPI
from pydantic import BaseModel
from FlashMCP import FlashMCP, Client # Import FlashMCP and Client
...
# 2. Create the FlashMCP server directly from the FastAPI app
# This is an async class method
async def create_mcp_server_from_fastapi():
    mcp_server = await FlashMCP.from_fastapi(
        app=api_app,
...
async def run_and_test():
    ...
    client = Client(server) # Uses FlashMCPTransport
    async with client:
...
3.  **OpenAPI Integration**: It calls `FlashMCP.from_openapi()`, passing the generated schema and the in-memory `httpx` client.
