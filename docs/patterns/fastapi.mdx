...
<Tip>
FlashMCP does *not* include FastAPI as a dependency; you must install it separately to run these examples.
</Tip>
...
mcp = FlashMCP.from_fastapi(app=app)
...
mcp = FlashMCP.from_fastapi(app=app, timeout=5.0)
```
...
For more details on route mapping or custom mapping rules, see the [OpenAPI integration documentation](/patterns/openapi#route-mapping); FlashMCP uses the same mapping rules for both FastAPI and OpenAPI integrations.
...
async def test():
    ...
    mcp = await FlashMCP.from_fastapi(app=app)
