...
# Calculator Service
calc_mcp = FlashMCP(name="CalculatorService")
...
FlashMCP supports two modes for mounting servers:
...
FlashMCP automatically uses proxy mounting when the mounted server has a custom lifespan, but you can override this behavior by explicitly setting `as_proxy=False` or `as_proxy=True`.

#### Interaction with Proxy Servers

When using `FlashMCP.from_client()` to create a proxy server, mounting that server will always use proxy mounting since the proxy server is already designed to be accessed via a client interface.

```python
from FlashMCP import FlashMCP, Client

# Create a proxy for a remote server
remote_proxy = FlashMCP.from_client(Client("http://example.com/mcp"))
...
app = FlashMCP(name="MainApplication")
...
text_mcp = FlashMCP(name="TextUtilities")
...
data_mcp = FlashMCP(name="DataAPI")
...
This pattern promotes code organization and reuse within your FlashMCP projects. 