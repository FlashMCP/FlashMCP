"""FlashMCP run command implementation."""
...
def create_client_server(url: str) -> Any:
    """Create a FlashMCP server from a client URL.

    Args:
        url: The URL to connect to

    Returns:
        A FlashMCP server instance
    """
    try:
        import FlashMCP

        client = FlashMCP.Client(url)
        server = FlashMCP.FlashMCP.from_client(client)
        return server
