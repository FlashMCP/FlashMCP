...
from FlashMCP.cli import claude
...
def _build_uv_command(
    ...
    """Build the uv run command that runs a FlashMCP server through FlashMCP run."""
    cmd = ["uv"]

    cmd.extend(["run", "--with", "FlashMCP"])
    ...
    # Add FlashMCP run command
    cmd.extend(["FlashMCP", "run", file_spec])
    return cmd
...
def _import_server(file: Path, server_object: Optional[str] = None):
    """Import a FlashMCP server from a file.
...
def run(
    ...
    all dependencies are available. For dependency management, use FlashMCP install
    or FlashMCP dev instead.
    """
    file, server_object = _parse_file_path(file_spec)
...
def install(
    ...
    """Install a FlashMCP server in the Claude desktop app.
