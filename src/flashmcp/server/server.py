...
from FlashMCP.server.http import (
...
@asynccontextmanager
async def default_lifespan(server: FlashMCP[LifespanResultT]) -> AsyncIterator[Any]:
...
def _lifespan_wrapper(
    app: FlashMCP[LifespanResultT],
    lifespan: Callable[
        [FlashMCP[LifespanResultT]], AbstractAsyncContextManager[LifespanResultT]
    ],
) -> Callable[
...
class FlashMCP(Generic[LifespanResultT]):
    def __init__(
        self,
        name: str | None = None,
        ...
        **settings: Any,
        ...
        if settings:
            ...
            warnings.warn(
                ...
                "to the FlashMCP constructor is deprecated (as of 2.3.4), "
        ...
        if resource_prefix_format is None:
            self.resource_prefix_format = (
                FlashMCP.settings.settings.resource_prefix_format
            )
        else:
    ...
    def custom_route(
        ...
        """
        Decorator to register a custom HTTP route on the FlashMCP server.
    ...
    async def run_sse_async(
        ...
        warnings.warn(
            ...
            "`FlashMCP.server.http.create_sse_app` and run it directly.",
            DeprecationWarning,
            stacklevel=2,
        )
    ...
    def sse_app(
        ...
        warnings.warn(
            ...
            "alternative, or call `FlashMCP.server.http.create_sse_app` directly.",
            DeprecationWarning,
            stacklevel=2,
        )
    ...
    def mount(
        ...
        *,
        ...
        """Mount another FlashMCP server on this server with the given prefix.
        ...
        Args:
            ...
            server: The FlashMCP server to mount.
        ...
        if as_proxy and not isinstance(server, FlashMCPProxy):
            server = FlashMCPProxy(Client(transport=FlashMCPTransport(server)))
    ...
    async def import_server(
        ...
        """
        Import the MCP objects from another FlashMCP server into this one,
        ...
        Args:
            ...
            server: The FlashMCP server to import
...
def add_resource_prefix(
    ...
    if prefix_format is None:
        prefix_format = FlashMCP.settings.settings.resource_prefix_format
