"""FlashMCP server implementation for OpenAPI integration."""
...
from FlashMCP.utilities.func_metadata import (
...
from FlashMCP.utilities.openapi import format_description_with_responses
...
class RouteType(enum.Enum):
    """Type of FlashMCP component to create from a route."""
...
@dataclass
class RouteMap:
    """Mapping configuration for HTTP routes to FlashMCP component types."""
...
def _determine_route_type(
    ...
    """
    Determines the FlashMCP component type based on the route and mappings.
...
class FlashMCPOpenAPI(FlashMCP):
    """
    FlashMCP server implementation that creates components from an OpenAPI schema.

    This class parses an OpenAPI specification and creates appropriate FlashMCP components
    (Tools, Resources, ResourceTemplates) based on route mappings.

    Example:
        ```python
        from FlashMCP.server.openapi import FlashMCPOpenAPI, RouteMap, RouteType
        import httpx
    ...
    def __init__(
        ...
        **settings: Any,
        ...
        """
        Initialize a FlashMCP server from an OpenAPI schema.
        ...
        logger.info(f"Created FlashMCP OpenAPI server with {len(http_routes)} routes")
