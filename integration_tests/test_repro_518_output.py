...
def test_server_shows_informative_error_on_stderr():
    """
    Runs a minimal FlashMCP+FastAPI server (that omits lifespan wiring)
        ...
        if process.poll() is None:
            try:
                with httpx.Client(timeout=5.0) as client:
                    # The mounted FlashMCP app is at root, its internal default path is /mcp
    ...
    assert "goFlashMCP.com/deployment/asgi" in normalized_stderr
